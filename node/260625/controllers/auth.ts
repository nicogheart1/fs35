...
const chiSono = (request: RequestWithUserI, response: Response) => {
  try {
    const { user } = request;
    response.status(200).send(`Tu sei ${user.username}`);
  } catch (error) {
    response.status(500).json({ msg: error });
  }
};
...
const authorize = (request: Request, response: Response, next: NextFunction) => {
  passport.authenticate("jwt", { session: false }, (error: any, user?: UserI) => {
    if (!user || error) {
      response.status(401).send("Non sei autorizzato");
    } else {
      request.user = user;
      next();
    }
...
export { login, registrazione, authorize, chiSono };
